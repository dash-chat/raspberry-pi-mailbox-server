...
# Validate Wi-Fi credentials before erasing anything: the Pi only validates
# at boot, when it's too late to notice a typo'd env file.
if [ -n "$envdir" ] && [ -f "$envdir/wifi.env" ]; then
  wifienv="$envdir/wifi.env"
